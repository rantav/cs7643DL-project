...
class StyleLoss(nn.Module):
    def gram_matrix(self, features, normalize=True):
        ...
        ##############################################################################
        #gram = torch.dot(torch.transpose(features), (features))

        N, C, H, W = features.shape
        reshaped_f = torch.reshape(features, (N*C, H*W))
        gram = torch.mm(reshaped_f, reshaped_f.t())
        if normalize:
          gram = gram / (N * C * H * W)
    ...
    def forward(self, feats, style_layers, style_targets, style_weights):
        ...
        ##############################################################################
        gram_matrices_for_feats =  [self.gram_matrix(feats[i]) for i in style_layers]

        loss = torch.tensor(0.0)
        for i in range(len(gram_matrices_for_feats)):
          loss += style_weights[i] * torch.nn.MSELoss(size_average=False)(style_targets[i], gram_matrices_for_feats[i])
